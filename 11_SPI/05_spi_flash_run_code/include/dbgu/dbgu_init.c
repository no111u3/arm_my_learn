...
static void clear_pdc_tx_rx_counters(void) {
...
// calculate divider
static unsigned calc_divider(unsigned int clock, unsigned int baudrate) {
...
// DBGU init
static void dbgu_init(unsigned int clock, unsigned int baudrate) {
    int divider;
    //enabling DBGU pio
    dbgu_pio_enable();
    ...
    // calculate divider
    divider = calc_divider(clock, baudrate);
