...
// TWI read
void twi_read(unsigned char device_id, unsigned short address, unsigned char * data, unsigned size) {
    unsigned status;
    // set twi master mode register
...
// TWI write
void twi_write(unsigned char device_id, unsigned short address, unsigned char * data, unsigned size) {
    unsigned status;
    // set twi master mode register
    AT91C_BASE_TWI->TWI_MMR = ((device_id << 16) | AT91C_TWI_IADRSZ_2_BYTE) & ~AT91C_TWI_MREAD;
    // set twi internal address register
